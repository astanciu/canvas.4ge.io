...
export default ({view}) => {
  ...
  const gridPattern = 
    <React.Fragment>
      <pattern id="smallGrid" width={SMALL_SPACING} height={SMALL_SPACING} patternUnits="userSpaceOnUse">
        <path className={styles.small} d={`M ${SMALL_SPACING} 0 L 0 0 0 ${SMALL_SPACING}`} fill="none" stroke="#eee" />
      </pattern>
      <pattern id="grid" width={BIG_SPACING} height={BIG_SPACING} patternUnits="userSpaceOnUse">
  ...
  <g className="grid">
   <defs>
    ...
    <rect x={-1* SIZE} y={-1* SIZE} width={SIZE * 2} height={SIZE * 2} fill={`url(#grid)`} />
  </g>
  )
}
